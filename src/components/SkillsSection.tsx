...
const SkillCard = ({ icon: Icon, label, i }: { icon: typeof Network; label: string; i: number }) => (
  <AnimatedSection delay={i * 0.08}>
    <motion.div
      whileHover={{ scale: 1.05, y: -6 }}
      className="glass glow-border p-6 rounded-xl text-center cursor-default group"
    >
      <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center mx-auto mb-4 group-hover:from-primary/30 group-hover:to-accent/30 transition-all">
  ...
  <section id="skills" className="section-padding relative">
    <div className="max-w-6xl mx-auto relative z-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {['Network Admin', 'Server Maintenance', 'Troubleshooting', 'CCTV Management', 'Documentation'].map((a, i) => (
          <AnimatedSection key={a} delay={i * 0.1}>
            <motion.div whileHover={{ scale: 1.03 }} className="glass glow-border px-5 py-4 rounded-xl text-center">
              <span className="text-foreground font-medium text-sm">{a}</span>
            </motion.div>
          </AnimatedSection>
        ))}
      </div>
